...
- ### ①存储引擎
   ...
   **MyISAM**: Indexed Sequential Access Method。从名字可以看出来索引是优势。但是不支持事务，没有行级锁，也不能安全恢复。<br/>
   **InnoDB**: 支持行锁，支持事务，外键，崩溃后安全恢复，但是不支持全文索引。

    > MyISAM更适合读密集的表，InnoDB更适合写密集的表。在数据库有主从分离的情况下，经常选择InnoDB作为主库的存储引擎。MyISAM支持压缩，所以对于数据量很大，且也不需要支持事务的表来说更好。(主从分离：主库处理事务性增、改、删操作，从库处理查询操作)
...
![事务的特性](https://user-gold-cdn.xitu.io/2018/5/20/1637b08b98619455?w=312&h=305&f=png&s=22430)
   ...
   2.  **一致性(Consistency)：** 执行事务前后，数据库从一个一致性状态转换到另一个一致性状态。特别在并发环境中始终一致的状态。不一致就是脏读、幻读等
...
- **SERIALIZABLE（串行）:** 	最高的隔离级别，完全服从ACID的隔离级别。所有的事务依次逐个执行，这样事务之间就完全不可能产生干扰，也就是说，**该级别可以防止脏读、不可重复读以及幻读**。但是这将严重影响程序的性能。通常情况下也不会用到该级别。
  ...
  **脏读**：一个事务读取到另外一个事务未提交的数据(修改完立即释放锁)
  **幻读**：一个事务内读到另一个事务插入的数据，导致前后不一致。
  **不可重复读**：一个事务读取到另一个事务已经提交的数据。A每次查询结果都收到B改动带来的影响，失去意义。
...
- ### ⑥锁机制与InnoDB锁算法
   ![MySQL锁分类](https://user-gold-cdn.xitu.io/2018/7/23/164c6d7ae44d8ac6?imageView2/0/w/1280/h/960/ignore-error/1)
